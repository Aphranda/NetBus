...
| ROUTe:SWITch# | CHAN?/CHAN, MODE?, IDEN?, OUTP?, INP?, COND? | 7 | 7 | 含通道切换验证 |
...
| # | 测试项 | 状态 | 耗时 | 详情 |
|---|--------|------|------|------|
| 1 | IEEE488.2 *IDN? | **PASS** | 216ms | IDN 正确: NetBus,PPA-NB100,00000000,v1.0.0 |
| 2 | IEEE488.2 *STB? | **PASS** | 205ms | STB = 0 |
| 3 | IEEE488.2 *OPC? | **PASS** | 205ms | OPC = 1 |
| 4 | IEEE488.2 *CLS | **PASS** | 202ms | 已执行 |
| 5 | IEEE488.2 *RST | **PASS** | 204ms | 已复位 |
| 6 | IEEE488.2 *WAI | **PASS** | 202ms | 已执行 |
| 7 | SYST:ERR? | **PASS** | 202ms | 错误队列为空 (No error) |
| 8 | SYST:ERR:COUN? | **PASS** | 204ms | 错误计数 = 0 |
| 9 | ATT A 读取 | **PASS** | 203ms | ATT A 读取 = 0 |
| 10 | ATT B 读取 | **PASS** | 202ms | ATT B 读取 = 0 |
| 11 | ATT 基准值 | **PASS** | 0ms | 当前: A=0, B=0 |
| 12 | ATT A<-3 | **PASS** | 215ms | OK |
| 13 | ATT B<-7 | **PASS** | 214ms | OK |
| 14 | ATT A<-12 | **PASS** | 215ms | OK |
| 15 | ATT B<-14 | **PASS** | 214ms | OK |
| 16 | ATT 验证最终值 | **PASS** | 0ms | A=12, B=14 |
| 17 | ATT A<-99 越界 | **PASS** | 204ms | 正确拒绝 (范围保护) |
| 18 | ATT A<--1 越界 | **PASS** | 201ms | 正确拒绝 (范围保护) |
| 19 | ATT 恢复原始值 | **PASS** | 0ms | 已恢复 |
| 20 | DET1 SN? | **PASS** | 219ms | SN: "PINPRFB00300000038" |
| 21 | DET1 VERS? | **PASS** | 205ms | 版本: 3.1.21 |
| 22 | DET1 FLAS:INFO? | **PASS** | 232ms | Flash 信息已返回 (含 JEDEC ID) |
| 23 | DET1 FLAS:DATA? @0x9001 | **PASS** | 215ms | 数据: ERROR: Flash read failed |
| 24 | DET1 LED | SKIP | 0ms | 写命令, 跳过以保护设备 |
| 25 | SENS:DET1:TEMP? | **PASS** | 219ms | 温度: -16119,-26870,0 |
| 26 | SENS:DET1:POW? | **PASS** | 202ms | 功率:  |
| 27 | CAN:SCAN? (1-5) | **PASS** | 203ms | 无响应 |
| 28 | CAN SEND (ID=0x101) | **PASS** | 263ms | CAN 响应: ERROR: Power query failed |
| 29 | SW1 IDEN? | **PASS** | 202ms | 响应:  |
| 30 | SW1 CHAN? | **PASS** | 356ms | 通道: 0 |
| 31 | SW1 MODE? | **PASS** | 202ms | 模式:  |
| 32 | SW1 OUTP? | **PASS** | 342ms | 无设备: 0 responses"PPA-SP10",0,1,1,0,21504<br>ERROR: Modbus read fail |
| 33 | SW1 INP? | **PASS** | 202ms | 无设备: ERROR: Read inputs failed (Slave address mismatch) |
| 34 | SW1 COND? | **PASS** | 204ms | 状态: ERROR: Read status failed (Sla |
| 35 | SW1 当前通道 | **PASS** | 0ms | 当前 CH=1 |
| 36 | SW1 CHAN 3 | **FAIL** | 206ms | 失败: ERROR: Write failed (Slave address mismatch) |
| 37 | SW1 验证 CH=3 | **FAIL** | 0ms | CH=3 验证失败 |
| 38 | SW1 恢复 CH=1 | **FAIL** | 0ms | 恢复失败 |
| 39 | STATus - OPERation | **PASS** | 203ms | 0 |
| 40 | STATus - QUEStionable | **PASS** | 204ms | 0 |
| 41 | DIAG:DEBUG? 初始 | **PASS** | 0ms | 当前: 1 |
| 42 | DIAG:DEBUG OFF | **PASS** | 206ms | 已关闭 |
...
| 47 | DIAG:ECHO OFF | **PASS** | 204ms | 已关闭 |
...
- **响应**:
```
ERROR: Write failed (Slave address mismatch)
```
...
- **响应**:
```
ERROR: Modbus read failed (Slave address mismatch)
```
...
- **请求**: `ROUT:SWIT1:OUTP?`
- **耗时**: 342ms
- **详情**: 无设备: 0 responses"PPA-SP10",0,1,1,0,21504<br>ERROR: Modbus read fail
- **响应**:
```
0 responses"PPA-SP10",0,1,1,0,21504
ERROR: Modbus read failed (Slave address mismatch)ERROR: Modbus read failed (Slave address mismatch)ERROR: Read coils failed (Slave address mismatch)
```
...
- **请求**: `ROUT:SWIT1:INP?`
- **耗时**: 202ms
- **详情**: 无设备: ERROR: Read inputs failed (Slave address mismatch)
- **响应**:
```
ERROR: Read inputs failed (Slave address mismatch)
```
...
- **响应**:
```
ERROR: Read status failed (Slave address mismatch)
```
...
- **请求**: `ROUT:SWIT1:CHAN 3`
- **耗时**: 206ms
- **详情**: 失败: ERROR: Write failed (Slave address mismatch)
- **响应**:
```
ERROR: Write failed (Slave address mismatch)
```
